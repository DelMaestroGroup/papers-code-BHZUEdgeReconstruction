...
set key width -3 samplen 0.1 spacing 0.7 at 0.9,0.0201
set label 1 '\scalebox{1.0}{(a)}' at -2.1,0.018
set label 2 '\scalebox{0.85}{$N_0$}' at 1.6,0.0185

plot "../data/GSE_vs_Sz_for_diff_Vo4.6.txt" u 1:((($2)+88.7462080)/1.0) w l lw 4.5 lt 1 dt 3 lc rgb '#abebc6' ti '',"" u 1:((($2)+88.7462080)/1.0) w p ps 1.0 pt 7 lc rgb '#239b56' ti '\scalebox{0.9}{$v_0=1.53$}', "" using ($1 -0.07):((($2)+88.7462080)/1.0):(0.14):(0) w vectors nohead lt 1 lw 3 lc rgb '#239b56' ti ''
unset ytics
unset key
unset label 1
unset label 2
...
set yr [-0.0015:0.015]
set ylabel '\fontsize{8}{8} $[E(S^z_{tot})-E_{GS}]\times 10^{-2}$' offset 5.95,0
...
plot "../data/GSE_vs_Sz_for_diff_Vo4.2.txt" u 1:((($2)+88.8977663)/1.0) w l lw 4.5 lt 1 dt 3 lc rgb  '#f1948a' ti '', "" u 1:((($2)+88.8977663)/1.0) w p ps 1.0 pt 7 lc rgb '#cb4335' ti '\scalebox{0.9}{$v_0=1.40$}',"" using ($1 -0.07):((($2)+88.8977663)/1.0):(0.14):(0) w vectors nohead lt 1 lw 3 lc rgb '#cb4335' ti ''
...
set yr [-0.001:0.010]
set xlabel '\fontsize{8}{8} $S^{z}_{tot}$' offset 1.5,1.15
...
#set key width -2 samplen 0.1 spacing 0.7 at 1.82,0.0097
set key width -2 samplen 0.1 spacing 0.7 at 0.85,0.0019
#set label 1 '\scalebox{1.0}{(c)}' at -1.5,0.0089
set label 1 '\scalebox{1.0}{(c)}' at -2.1,0.0013
set label 2 '\scalebox{0.85}{$N_0+2$}' at 1.4,0.0013

plot "../data/GSE_vs_Sz_for_diff_Vo4.0.txt" u 1:((($2)+89.0718564)/1.0) w l lw 4.5 lt 1 dt 3 lc rgb '#aed6f1' ti '', "" u 1:((($2)+89.0718564)/1.0) w p ps 1.0 pt 7 lc rgb '#2e86c1' ti '\scalebox{0.9}{$v_0=1.33$}',"" using ($1 -0.07):((($2)+89.0718564)/1.0):(0.14):(0) w vectors nohead lt 1 lw 3 lc rgb '#2e86c1' ti ''
